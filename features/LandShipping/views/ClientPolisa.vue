<template>
  <div>
    ...
    <v-dialog v-model="addEmployee" width="500">
      <v-card class="pa-5">
        <h2 class="text-center">إضافة موظف</h2>
      ...
      <v-card>
        ...
        <v-data-table :headers="headersEmployee" :items="employee" :search="search">
          <template v-slot:[`item.img`]="{ item }">
            ...
            <v-icon color="success" small @click="ref(item)">mdi-camera</v-icon>
            <input
              class="d-none"
              ref="uploadImg"
              type="file"
          ...
          <template v-slot:[`item.actions`]="{ item }">
            <v-icon color="success" small @click="editEmployee(item)">mdi-pen</v-icon>
            <v-icon color="error" small @click="opendelEmployee(item)">mdi-delete</v-icon>
          </template>
        </v-data-table>
  ...
  methods: {
    ...
    async addNewEmployee() {
      ...
      if (this.user._id) {
        ...
        try {
          this.loading = true;
          const res = await axios.post("https://resturant-appv1.herokuapp.com/employee", this.user);
          console.log(res);
          this.loading = false;
          this.addEmployee = false;
